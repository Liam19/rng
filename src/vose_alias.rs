    ...
    T: Display + Copy + Hash + Eq + Debug,
{
    ...
    fn roll_die_and_flip_coin(&self, rng: &mut Rng) -> (T, u16) {
        let i: T = match self.elements.sample(rng) {
            Some(e) => *e,
            None => panic!("Internal error. The element vector is empty."),
        };
    ...
    fn select_element(&self, die: T, coin: u16) -> T {
        ...
        let p_i: f32 = match self.prob.get(&die) {
            Some(p) => *p,
            None => panic!("Internal error. The probability vector is empty."),
        };
        ...
        if (coin as f32) <= (p_i * 100.0) {
            ...
            match self.alias.get(&die) {
                Some(alias_i) => *alias_i,
                None => panic!("Internal error. No alias found for element {die:?}."),
            }
        }
    }
}
    ...
    T: Display + Copy + Hash + Eq + Debug,
{
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for k in self.alias.keys() {
            let a: T = match self.alias.get(k) {
                Some(element) => *element,
                None => panic!("Internal error. The alias map does not contain element for {k}."),
            };

            str_alias = str_alias + &format!("{k}:{a}, ");
        }
        ...
        for k in self.prob.keys() {
            let p: f32 = match self.prob.get(k) {
                Some(element) => *element,
                None => panic!("Internal error. The alias map does not contain element for {k}."),
            };

            str_prob = str_prob + &format!("{k}:{p:.2}, ");
        }
        ...
        write!(
            f,
            "{{ elements: {str_elements}, alias: {str_alias}, prob: {str_prob}}}"
        )
    }
}
...
mod tests {
    ...
    fn test_roll_die_flip_coin() {
        ...
        let (die, coin) = va.roll_die_and_flip_coin(Rng::new());
